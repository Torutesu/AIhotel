...
これらは開発・デモ用で、**本番には入れません**。seed は `NODE_ENV=production` では実行を拒否します
（デモ環境で意図的に入れるときだけ `ALLOW_SEED_IN_PRODUCTION=true`）。本番の最初の運営は
`job create-platform-admin` で作ります（`docs/運用手順書.md` §1.1）。
...
- `pnpm --filter backend job:create-platform-admin --email <メール> --name <名前>`（本番コンテナは `job create-platform-admin --email ... --name ...`）—
  **定期実行ではなく、本番を立ち上げるときに1回だけ**使う。最初の運営（PLATFORM_ADMIN）を一時パスワードつきで作る（R-2-5）。
  メール送信が有効なら本人に届け、無効なら標準出力に1回だけ表示する。初回ログインでパスワードの変更を求められる
