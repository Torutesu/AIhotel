...
6. **ティア**: ホテルごとに「おまかせ（自動採用・結論だけ）／スタンダード（要点と理由）／エンタープライズ（内訳・候補・信頼区間まで）」を個社MDの「ティア:」か設定タブで決める。アルゴリズムは同じで、説明の深さと自動採用の既定だけが変わる。販売代理店向けの説明とマトリックスは `docs/販売代理店向け説明.md`。
7. **会話フィードバック**: `docs/knowledge/` の Markdown（レベニューマネジメントの基礎資料）を章単位で引用しながら、チャットで「なぜこのランクか」「今日何をすべきか」に答える。「9/20 は近くで工事だから下げて」「競合Aの売止めは団体」「連休前はもっと強気で」のような発話はイベント登録・採否記録・係数調整・売止め除外として実行され、監査ログと会話に残る。`ANTHROPIC_API_KEY` か `OPENAI_API_KEY` が必要。
