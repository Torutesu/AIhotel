...
// 実績データの取り込み（#82 / POST /imports/daily-data）
// PMS 連携（Phase 4）までのつなぎ。CSV を読み込み、まず dryRun で検証して件数を見せ、
// 利用者が確認してから取り込む。1行でも不正ならバックエンドは何も書き込まない。
...
import {
  DAILY_DATA_CSV_TEMPLATE,
  parseDailyDataCsv,
  type CsvParseError,
  type DailyDataCsvRow,
} from "@/lib/daily-data-csv"
...
export function DailyDataImportSection() {
  const { hotelId, hotel, user } = useAuth()
  ...
  const downloadTemplate = () => {
    ...
    const blob = new Blob(["﻿" + DAILY_DATA_CSV_TEMPLATE], { type: "text/csv;charset=utf-8" })
    const url = URL.createObjectURL(blob)
    const a = document.createElement("a")
    a.href = url
    a.download = "日次実績テンプレート.csv"
    a.click()
  ...
  const handleFile = async (file: File) => {
    ...
    const parsed = parseDailyDataCsv(text)
    if (parsed.errors.length > 0) {
      setErrors(parsed.errors)
      return
    }
    if (parsed.rows.length > MAX_ROWS) {
      setErrors([{ line: 1, message: `1回に取り込めるのは${MAX_ROWS}行までです（${parsed.rows.length}行あります）。ファイルを分けてください` }])
      return
    }

    setChecking(true)
    try {
      const result = await api.importDailyData(hotelId, parsed.rows, true)
  ...
  const handleImport = async () => {
    ...
    setImporting(true)
    try {
      const result = await api.importDailyData(hotelId, preview.rows, false)
      toast.success(`${result.total}行を取り込みました`, {
        description: `新規 ${result.created}行・更新 ${result.updated}行（${result.startDate} 〜 ${result.endDate}）`,
      })
      reset()
    } catch (err) {
      if (err instanceof ApiClientError && err.fieldErrors.length > 0) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>実績データの取り込み</CardTitle>
        <CardDescription>
            ...
            {preview && (
              <div className="space-y-3 rounded-lg border p-3 text-sm">
                ...
                <p className="text-muted-foreground">
                  新規 {preview.created}行・既存の実績を上書き {preview.updated}行
                </p>
